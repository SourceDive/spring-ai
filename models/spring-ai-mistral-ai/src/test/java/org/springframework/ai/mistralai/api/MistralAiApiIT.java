...
class MistralAiApiIT {
	...
	void chatCompletionEntityWithTextChunkAndImageUrlChunkContent() throws IOException {
		...
		var content = extractTextContent(chatCompletionMessage);
	...
	void chatCompletionEntityWithThinkingModel(MistralAiApi.ChatModel chatModel) {
		...
		var thinkingContent = extractThinkingTextContent(chatCompletionMessage);
	...
	void chatCompletionStreamWithThinkingModel(MistralAiApi.ChatModel chatModel) {
		...
		var hasContent = hasContent(chatCompletionChunks, ChatCompletionMessage::extractTextContent);
		assertThat(hasContent).isTrue();
		var hasThinkingContent = hasContent(chatCompletionChunks, ChatCompletionMessage::extractThinkingTextContent);
	...
	private static String extractReferencesAsJsonString() {
		return new BasicJsonTester(MistralAiApiIT.class).from(new ClassPathResource("references.json")).getJson();
	}
